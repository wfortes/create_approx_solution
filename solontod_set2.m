    ...
    for img_index = img_index_set;
        for type_code = type_set;
            for N_proj = N_proj_set;
                ...
                x = box_constraint_solver(M,Q,1e+5*length(Q),0.1,'ART');
...
%                 address ='/ufs/fortes/Desktop/PhD_m_files/tomography/create_approx_solution/reconstruction/tol-001/';
%                 filename = strcat(address,'d_sol-','Im',img,'-sz',sz,'-proj',proj,'-',type,'tol-001');
%                 load(filename);
% %                 figure;
% %                 imshow(x);
%                 x = reshape(reconstruction,32,32);
%                 newfilename = strcat(address,'rec-','Im',img,'-sz',sz,'-proj',proj,'-',type,'tol-001.fig');
%                 figura = imshow(x);
%                 save(newfilename,'figura');
% 
